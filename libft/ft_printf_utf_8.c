...
/*   Updated: 2018/08/11 09:48:12 by rsharipo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void	utf_8_size(unsigned int *ws, int size, int *chars)
{
	int	i;
...
void	encode(unsigned int *ws, char *s, int chars, int j)
{
	int	tmp;
...
char	*convert_to_utf_8(unsigned int *ws, int size, int prec)
{
	...
	chars = 0;
	if (ws)
		utf_8_size(ws, size, &chars);
	if (prec > 0 && prec < chars)
		chars = prec;
	if (chars && (s = (char *)malloc(chars + 1)))
	{
		encode(ws, s, chars, 0);
		return (s);
	}
